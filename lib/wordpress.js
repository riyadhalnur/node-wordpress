...
extend( Client.prototype, {
	...
	getMediaLibrary: function ( filter, fn ) {
		...
		if( filter.id ) {
			filter.parent_id = filter.id;
			delete filter.id;
		}
		if( filter.mime ) {
			filter.mime_type = filter.mime;
			delete filter.mime;
		}
		this.authenticatedCall( "wp.getMediaLibrary", filter, function ( error, medias ) {
			if ( error ) {
				return fn( error );
			}

			fn( null, medias.map(function( media ) {
				return fieldMap.from( media, "media" );
			}));
		});
	},
